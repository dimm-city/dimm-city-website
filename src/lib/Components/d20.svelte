<style>
	...
	.die.rolling {
		-webkit-animation: roll var(--d20-roll-duration) linear;
		animation: roll var(--d20-roll-duration) linear;
	}
	.die[data-face='1'] {
	...
	:root {
		--d20-roll-duration: 3s;
		--d20-face-color: var(--primary-accent); /* rgba(20, 129, 180, 0.75);*/
		--d20-font-color: var(--secondary-accent);
		--d20-shadow-color: var(--primary-accent);
		--d20-result-face-color: var(--primary-accent);
		--d20-result-font-color: var(--secondary-accent);
		--d20-result-shadow-color: black;
	}
	.die .face {
		border-bottom-color: var(--d20-face-color);
		opacity: 0.7;
	}
	.die .face:before {
		color: var(--d20-font-color);
		text-shadow: 1px 1px 1px var(--d20-shadow-color);
	}

	.die .face.active {
		opacity: 0.8;
		border-bottom-color: var(--d20-result-face-color);
	}
	.die .face.active:before {
		color: var(--d20-result-font-color);
		text-shadow: 1px 1px 1px var(--d20-result-shadow-color);
	}
</style>
...
<div class="content">
	...
	>
		<figure class="face face-1" class:active={face == 1} />
		<figure class="face face-2" class:active={face == 2} />
		<figure class="face face-3" class:active={face == 3} />
		<figure class="face face-4" class:active={face == 4} />
		<figure class="face face-5" class:active={face == 5} />
		<figure class="face face-6" class:active={face == 6} />
		<figure class="face face-7" class:active={face == 7} />
		<figure class="face face-8" class:active={face == 8} />
		<figure class="face face-9" class:active={face == 9} />
		<figure class="face face-10" class:active={face == 10} />
		<figure class="face face-11" class:active={face == 11} />
		<figure class="face face-12" class:active={face == 12} />
		<figure class="face face-13" class:active={face == 13} />
		<figure class="face face-14" class:active={face == 14} />
		<figure class="face face-15" class:active={face == 15} />
		<figure class="face face-16" class:active={face == 16} />
		<figure class="face face-17" class:active={face == 17} />
		<figure class="face face-18" class:active={face == 18} />
		<figure class="face face-19" class:active={face == 19} />
		<figure class="face face-20" class:active={face == 20} />
	</div>
</div>
...
<script>
	...
	function roll() {
		...
		timeoutId = setTimeout(() => {
			rolling = false;
			face = Math.floor(Math.random() * sides) + initialSide;
			clearTimeout(timeoutId);
		}, animationDuration);
		return false;
	}
